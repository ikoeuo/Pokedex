...
app.put('/api/items/:id', (req, res) => {
  const { id } = req.params;
  const itemIndex = items.findIndex(item => item.pokedex_number === id);
  if (itemIndex === -1) {
    return res.status(404).send('Item not found.');
  }
...
app.patch('/api/items/:id', (req, res) => {
  const { id } = req.params;
  const item = items.find(item => item.pokedex_number === id);
  if (!item) {
    return res.status(404).send('Item not found.');
  }
...
app.delete('/api/items/:id', (req, res) => {
  ...
  items = items.filter(item => item.pokedex_number !== id);
  saveItems(items); // Persist the updated items array to the database
